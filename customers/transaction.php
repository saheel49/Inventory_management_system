...
$db = getDB();
$cs = $db->prepare('SELECT * FROM customers WHERE id=? LIMIT 1');
$cs->bind_param('i',$customer_id); $cs->execute();
$customer = $cs->get_result()->fetch_assoc(); $cs->close();
...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    $vids = $_POST['variety_ids']  ?? [];
    $qtys = $_POST['quantities']   ?? [];
    $items = [];
    foreach($vids as $i=>$vid){
        $vid = (int)$vid;
        $qty = (float)str_replace(',','.',trim($qtys[$i]??'0'));
    ...
    if(empty($errors)){
        foreach($items as $vid=>$qty){
            ...
            if(!$variety) continue;
            $up=(float)($variety['unit_price']??0); $ta=$qty*$up; $si=0.0; $so=$qty;
            ...
            $ls->bind_param('issssddsi',$vid,$date,$customer['name'],$inv,$del,$si,$so,$rem,$uid);
            ...
            $ct=$db->prepare('INSERT INTO customer_transactions (customer_id,variety_id,ledger_id,quantity,unit_price,total_amount,transaction_date,notes) VALUES (?,?,?,?,?,?,?,?)');
            $ct->bind_param('iiidddss',$customer_id,$vid,$lid,$qty,$up,$ta,$date,$rem);
            $ct->execute(); $ct->close();
        }
        log_activity('customer_out','Customer transaction for '.$customer['name']);
...
<div class="page-header">
  <div class="page-header-left">
    <h1><i class="fa-solid fa-arrow-right-arrow-left"></i> Customer Transaction</h1>
...
<!-- Entity banner -->
<div class="trx-band" style="margin-bottom:20px;">
  <div class="trx-avatar"><i class="fa-solid fa-user" style="color:#fff;font-size:20px;"></i></div>
  <div>
    <div class="trx-name"><?php echo htmlspecialchars($customer['name']); ?></div>
    <div class="trx-type">Customer &middot; Stock OUT Transaction</div>
    <?php if($customer['phone']): ?>
    <div style="font-size:11px;color:rgba(255,255,255,0.55);margin-top:2px;"><i class="fa-solid fa-phone" style="margin-right:4px;"></i><?php echo htmlspecialchars($customer['phone']); ?></div>
    <?php endif; ?>
  </div>
</div>
  ...
  <form method="POST">
    ...
    <div class="card" style="border:1px solid var(--border);padding:18px;margin-bottom:18px;">
      <div class="card-header" style="margin-bottom:14px;">
        <div class="card-title"><i class="fa-solid fa-cart-shopping"></i> Items (Stock OUT)</div>
      </div>
      ...
      <div style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;margin-bottom:12px;">
        ...
        <div style="align-self:flex-end;">
          <button type="button" id="addItemBtn" class="btn btn-primary"><i class="fa-solid fa-plus"></i> Add Item</button>
        </div>
      </div>
        ...
        <div class="tbl-scroll">
          <table id="itemsTable">
            ...
            <tbody>
              ...
              $vlookup=[]; foreach($varieties as $v) $vlookup[$v['id']]=$v;
              foreach($posted as $vid=>$qty): if(!isset($vlookup[$vid])) continue; $v=$vlookup[$vid]; $ns=$v['current_stock']-$qty; ?>
              <tr data-item-id="<?php echo $v['id']; ?>">
                ...
                <td class="number-col new-stock <?php echo $ns<0?'text-danger':''; ?>"><?php echo number_format($ns,3); ?></td>
    ...
    <div class="form-actions">
      ...
      <button type="submit" class="btn btn-primary"><i class="fa-solid fa-floppy-disk"></i> Record Transaction</button>
    </div>
  </form>
</div>
</div>
...
inp.addEventListener('input',function(){
  ...
  drop.innerHTML=m.slice(0,10).map(v=>`<div class="ac-item" data-id="${v.id}">
    ...
    <div class="ac-meta">Stock: ${parseFloat(v.current_stock).toFixed(3)} &middot; ${v.unit||'pcs'}</div>
  </div>`).join('');
...
drop.addEventListener('click',function(e){
  ...
  inp.value=v.product_name+' – '+v.variety_name;
  inp.dataset.selectedId=v.id; clrDrop();
});
...
document.getElementById('addItemBtn').addEventListener('click',function(){
  ...
  if(document.querySelector('#itemsTable tbody tr[data-item-id="'+id+'"]')){ showToast('warning','Duplicate','This item is already added.'); return; }
  var v=fv(id); if(!v){ showToast('error','Error','Item not found.'); return; }
  ...
  row.innerHTML=`<td>${v.product_name}<input type="hidden" name="variety_ids[]" value="${id}"></td>
    ...
    <td class="number-col new-stock${parseFloat(ns)<0?' text-danger':''}">${ns}</td>
    <td><button type="button" class="btn btn-danger btn-sm remove-item"><i class="fa-solid fa-xmark"></i></button></td>`;
...
document.querySelector('#itemsTable tbody').addEventListener('input',e=>{
  if(e.target.classList.contains('row-qty')){
    ...
    var s=parseFloat(e.target.dataset.stock)||0;
    var q=parseFloat(e.target.value)||0;
    var ns=(s-q).toFixed(3);
    var td=tr.querySelector('.new-stock');
    td.textContent=ns;
    td.classList.toggle('text-danger',parseFloat(ns)<0);
  }
});
</script>
